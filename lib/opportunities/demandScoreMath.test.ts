...
import { computeShortageScore, filterPlausibleLocations, nearestArea } from "./demandScoreMath";
...
describe("filterPlausibleLocations", () => {
  it("keeps points that are all at distinct coordinates", () => {
    const points = [
      { lat: 23.78, lng: 90.41 },
      { lat: 23.79, lng: 90.42 },
      { lat: 23.8, lng: 90.43 },
    ];
    expect(filterPlausibleLocations(points)).toHaveLength(3);
  });

  it("drops a large exact-duplicate cluster (a hardcoded fallback point, not real GPS demand)", () => {
    const fallback = { lat: 23.7808, lng: 90.4194 };
    const points = [
      ...Array.from({ length: 45 }, () => ({ ...fallback })),
      { lat: 23.79, lng: 90.42 },
      { lat: 23.8, lng: 90.43 },
    ];
    const filtered = filterPlausibleLocations(points);
    expect(filtered).toHaveLength(2);
    expect(filtered.every((p) => p.lat !== fallback.lat)).toBe(true);
  });

  it("keeps a small amount of coincidental duplication (default threshold: 3)", () => {
    const point = { lat: 23.75, lng: 90.4 };
