...
export function CorporateBookingsList({
  ...
  useEffect(() => {
    // fetchBookings only ever sets state after its `await fetch(...)`
    // resolves — a real async boundary, not the render-time derived
    // state this lint rule is meant to catch — it's just not written
    // as a `.then()` callback the linter's static analysis can see
    // through.
