...
export default function WorkerSOSAlerts({ workerId }: { workerId: string }) {
  ...
  const accept = async (sosId: string) => {
    ...
    try {
      ...
      if (res.status === 409) {
        setErrorMessage("Too slow — another worker already accepted this job.");
        setAlerts((prev) => prev.filter((a) => a.sosId !== sosId));
        return;
      }

      // Guard against non-200 responses (e.g. 401/403 if not signed in as
      // this worker, or 405 Method Not Allowed)
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        setErrorMessage(data?.error || `Failed to accept SOS (${res.status})`);
        return;
      }
  ...
  // 2. Render nothing when no alerts (and no error to show)
  if (alerts.length === 0 && !errorMessage) return null;
  ...
  return (
    <div style={{ border: "2px solid #dc2626", borderRadius: 12, padding: 16, marginTop: 16 }}>
      ...
      {errorMessage && (
        <p style={{ marginTop: 8, fontSize: 13, color: "#dc2626" }}>{errorMessage}</p>
      )}
