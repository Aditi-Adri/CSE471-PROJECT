...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Syncing from an external system (sessionStorage) on mount, not
    // reacting to a prop/state change — the case React's own docs
    // call out effects as the right tool for. Server render always
    // starts empty (see the comment above), so this can't run any
    // earlier without reintroducing the hydration mismatch it exists
    // to avoid.
