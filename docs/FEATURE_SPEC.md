...
| SMS (Twilio) | Twilio SMS API | **Real send path exists in [lib/twilio.ts](../lib/twilio.ts), intentionally left disconnected — this is final, not a TODO.** Twilio requires a verified payment method before a trial account can claim *any* phone number, even one paid for out of trial credit — conflicts with the free-APIs-only constraint everything else in this row follows. The team decided against adding a card for this project. Leave `TWILIO_ACCOUNT_SID`/`AUTH_TOKEN`/`FROM_NUMBER` unset (see `.env.example`) and it logs to the console instead; the dispatch logic in `server.ts` runs identically either way. |
