...
export default function ShopPage() {
  ...
  // Fetches inline (not via a separately-called named function) so
  // every setState call sits inside a .then()/.finally() callback,
  // not the effect body's own synchronous execution — same shape as
  // components/reviews/ReviewForm.tsx's fetch-on-mount effect.
  useEffect(() => {
    ...
    if (!userId) return;
    fetch(`/api/shop/orders/worker/${userId}`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => setOrders(data?.orders ?? []))
      .catch((err) => console.error("Failed to load order history:", err));
  }, [status, session?.user?.id, syncStock]);
