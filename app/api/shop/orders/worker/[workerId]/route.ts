...
export async function GET(
  ...
  try {
    ...
    // Auth was missing here — anyone could read anyone else's purchase
    // history by guessing/iterating ids. Restricted to the account's
    // own history (or an admin), same "who's asking" gate every other
    // per-user route in this app enforces.
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) {
      return NextResponse.json({ error: "You must be signed in." }, { status: 401 });
    }
    if (session.user.id !== workerId && session.user.role !== "ADMIN") {
      return NextResponse.json({ error: "Not your order history." }, { status: 403 });
    }
